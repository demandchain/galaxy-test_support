...
module TestSupport
  class CapybaraDiagnostics
    ...
    def self.diagnose_finder(test_object, function_name, *args)
      find_action = FindAction.new(test_object, function_name, *args)

      find_action.run
      generate_diagnostics_report "Diagnostic report on \"#{@function_name.to_s}\":", nil
    end
    ...
    class FindAction
          ...
          generate_diagnostics_report "An error occurred while processing \"#{@function_name.to_s}\":", $! do |report_table|
            if retry_action_with_found_element report_table
              ...
              if alternate_action_with_found_element report_table
                TestSupport::CapybaraDiagnostics.output_page_details "#{DateTime.now.strftime("%Y_%m_%d")}_failure_#{SecureRandom.uuid}.png"
                @return_value
              else
                raise $!
              end
      ...
      def all_elements report_table = nil
        unless @all_elements
          if options && options.has_key?(:from)
            ...
            sub_report  = TestSupport::DiagnosticsReportBuilder::ReportTable.new
      ...
      def analyze_report_element(element, report_table)
        element_report = TestSupport::DiagnosticsReportBuilder::ReportTable.new
