  ...
  class LogCapture
    class << self
        ...
        if report_table
          ...
          TestSupport::DiagnosticsReportBuilder.current_report.within_section("Log Dump:") do |report|
            report.within_table do |new_report_table|
              TestSupport::LogCapture.capture_logs new_report_table
            end
          end
        end
      end
      ...
      def highlight_log_output(log_text)
        output_text = TestSupport::DiagnosticsReportBuilder.format_code_refs(log_text)
      ...
      def output_log_file(report_table, log_file_name, options = {})
        ...
        begin
          options.reverse_merge!({ num_lines: TestSupport::Configuration.num_lines })

          num_lines  = options[:num_lines] || TestSupport::Configuration.num_lines
          num_lines  = TestSupport::Configuration.num_lines if num_lines <= 0
          log_buffer = ""
          ...
          if log_buffer
            ...
            report_table.write_stats File.basename(log_file_name),
                                     TestSupport::PrettyFormatter.format_string(
                                         TestSupport::LogCapture.highlight_log_output(
